...
contract Option {
    ...
    function publishOption(uint256 _optionId) external {
        CallOption storage option = options[_optionId];
        require(msg.sender == option.seller, "Only the seller can publish the option");
        require(!option.published, "Option has already been published");
    ...
    function buyOption(uint256 _optionId) external payable {
        CallOption storage option = options[_optionId];
        require(option.published, "Option must be published before it can be bought");
    ...
    function sellOption(uint256 _optionId, address _newBuyer) external {
        CallOption storage option = options[_optionId];
        require(option.published, "Option must be published before it can be sold");
